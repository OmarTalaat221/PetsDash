...
export default function AddAnimal() {
  ...
  const approveAnimal = async () => {
    setLoading(true);
    try {
      const res = await axios.post(
        `${globa_base_url}animals/admin_change_animal_status/${rowData.id}?token=${petsDashUser?.access_token}`,
        {
          status: "approved",
        }
      );
      ...
      if (res.status === 200) {
        ...
        handleGetAllAnimals();
        toast.success("Animal aprobado con éxito");
      }
    } catch (error) {
      handleErrorResponse(error);
    }
  ...
  const columns = [
...

    {
      ...
      key: "raza",
      render: (text, row) => (
        <div className="text-center text-capitalize">{row?.raza?.title_es}</div>
      ),
    },
    {
      title: "Departmento",
    ...
    },
    {
      ...
      key: "distrito",
      render: (text, row) => (
        <div className="text-center">{row?.distrito?.title_es}</div>
      ),
    },
      ...
      if (searchQuery?.length >= 1) {
        const newData = originalAnimals?.filter((item) => {
          if (
            ...
            !item?.f_name
              ?.toLowerCase()
              ?.includes(searchQuery?.toLowerCase()) && // good
            !item?.l_name?.toLowerCase().includes(searchQuery?.toLowerCase()) && // good
            !item?.sex?.toLowerCase()?.includes(searchQuery?.toLowerCase()) && // good
            !item?.size?.toLowerCase()?.includes(searchQuery?.toLowerCase()) && //good
            !item?.coat_color
              ?.toLowerCase()
              ?.includes(searchQuery?.toLowerCase()) && // good
            !item?.departmento?.title_es
              ?.toLowerCase()
              ?.includes(searchQuery?.toLowerCase()) &&
            !item?.raza?.title_es
              ?.toLowerCase()
              ?.includes(searchQuery?.toLowerCase()) &&
            !item?.provincia?.title_es
              ?.toLowerCase()
              ?.includes(searchQuery?.toLowerCase()) &&
            !item?.distrito?.title_es
              ?.toLowerCase()
              ?.includes(searchQuery?.toLowerCase())
          ) {
            return false;
  ...
  return (
    <>
      ...
      <Modal
        title={"Confirmar aprobar animal"}
        show={approveModal}
        onClose={() => setApproveModal(false)}
        showCloseBtn={true}
        confirmButton={{
          onClick: approveAnimal,
          children: loading ? (
            <div className="d-flex justify-content-center align-items-center">
              <ClipLoader size={20} color="#fff" loading={loading} />
            </div>
