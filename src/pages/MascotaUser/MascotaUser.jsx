...
import { edit, userPlus } from "../../assets/svgIcons";
...
export default function Mascota() {
    ...
    if (
      ...
      if (searchValue.length >= 1) {
        const newData = originalData.filter((item) => {
          ...
          if (
            searchValue &&
            !item?.useranimal?.animal?.f_name
              ?.toLowerCase()
              ?.includes(searchValue?.toLowerCase()) && // good
            !item?.useranimal?.animal?.l_name
              ?.toLowerCase()
              ?.includes(searchValue?.toLowerCase()) && // good
            !item?.user?.email
              ?.toLowerCase()
            ...
            !item?.user?.name
              ?.toLowerCase()
              ?.includes(searchValue?.toLowerCase()) && //good
            !item?.micro?.toLowerCase()?.includes(searchValue?.toLowerCase()) && // good
            !String(item?.id)
              ?.toLowerCase()
              .includes(searchValue?.toLowerCase()) //good
          ) {
  ...
  const handleGetData = async () => {
    setLoading(true);
    try {
      const res = await axios.post(`${base_url}show_agrag `, {
        user_id: petUser?.id,
      });
  ...
  const columns = [
...

    {
      ...
      render: (text, row) => (
        <div className="text-center">{row?.useranimal?.animal?.f_name}</div>
      ),
    },
    {
      title: "Apellidos",
    ...
    },
    {
      title: "DIRECCIÓN",
      render: (text, row) => (
        <div className="text-center">{row?.useranimal?.animal?.address}</div>
      ),
    },
    ...
    },
    {
      title: "Referencia",
      render: (text, row) => (
        <div className="text-center">{row?.referencia}</div>
      ),
    },
    // progress...
    {
